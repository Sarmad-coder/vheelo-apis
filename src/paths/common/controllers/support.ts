...
export const findByRider = async (req: Request, res: Response, next: NextFunction) => {
    try {
        return res.json({ status: "success", data: await SupportService.findByQuery({sender:req.params.rider,sendTo:"rider"})})
    } catch (error) {
        return next(new ValidationError("Invalid Id", error));
    }
}
